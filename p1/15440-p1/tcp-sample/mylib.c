...
int __xstat(int ver, const char * path, struct stat * stat_buf) {
	...
	orig_close(sock);
	errno = err;
    if (return_val != -1){ // keep return_val the same if it eqauls to -1, otherwise 0
    	return_val = 0;    // since return val is used as payloadsize is our case  	                   
    }                      // this can recover return_val to originial value
